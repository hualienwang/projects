...
class OrderProcessingInput(BaseModel):
    ...
    items: List[dict] = Field(..., description="商品列表，每个元素包含 product_id 和 quantity", examples=[{"product_id": 1, "quantity": 2}])
...
class ValidateOrderOutput(BaseModel):
    """验证订单节点输出"""
    is_valid: bool = Field(..., description="订单是否有效")
    validated_items: List[dict] = Field(..., description="验证后的商品列表")
...
class CheckOrderInventoryOutput(BaseModel):
    """检查库存节点输出"""
    inventory_status: str = Field(..., description="库存状态: sufficient/insufficient/partial")
    inventory_check_result: dict = Field(..., description="库存检查详情")
...
class CreateOrderOutput(BaseModel):
    ...
    order_created: bool = Field(..., description="订单是否创建成功")
...
class DeductInventoryOutput(BaseModel):
    """扣减库存节点输出"""
    inventory_updated: bool = Field(..., description="库存是否更新成功")
...
class GenerateShippingOrderOutput(BaseModel):
    """生成发货单节点输出"""
    shipping_order: dict = Field(..., description="发货单信息")
...
class NotifyCustomerOutput(BaseModel):
    """通知客户节点输出"""
    notified: bool = Field(..., description="是否通知成功")
